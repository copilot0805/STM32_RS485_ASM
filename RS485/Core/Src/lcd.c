...
// Địa chỉ của Bank1 (NE1), nơi LCD được kết nối
...
#define LCD_REG_ADDR (FSMC_BANK1_BASE_ADDR)

// Địa chỉ để ghi/đọc Dữ liệu (RAM) (RS=1)
#define LCD_RAM_ADDR (FSMC_BANK1_BASE_ADDR + LCD_RS_PIN_OFFSET)
...
/**
 * @brief  Tô một vùng chữ nhật với 1 màu
 */
void lcd_fill(uint16_t xsta, uint16_t ysta, uint16_t xend, uint16_t yend,
		uint16_t color) {
	uint32_t i, total_points;
	total_points = (uint32_t)(xend - xsta + 1) * (uint32_t)(yend - ysta + 1);
...
/**
 * @brief  Vẽ 1 đường thẳng (Thuật toán Bresenham)
 */
...
/**
 * @brief  Vẽ 8 điểm đối xứng cho hình tròn
 */
...
/**
 * @brief  Vẽ hình tròn (Thuật toán Bresenham)
 */
