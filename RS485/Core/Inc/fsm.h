...
/**
 * @brief  Khởi tạo FSM.
 * (Hàm này sẽ khởi tạo LCD và hiển thị layout AUTO ban đầu).
 */
void FSM_Init(void);

/**
 * @brief  Hàm chạy chính của FSM, được gọi liên tục trong while(1).
 * (Hàm này chứa logic timer cho chế độ AUTO và timeout).
 */
void FSM_Run(void);

/**
 * @brief  Hàm xử lý sự kiện nút nhấn (được gọi mỗi 10ms).
 * (Hàm này kiểm tra button_count[] và thực thi logic FSM).
 */
